const http_proxy = require ('@pepmartinez/http-proxy');
const util =       require ('util');
const _ =          require ('lodash');
const Log =        require ('winston-log-space');
const CBuffer =    require ('CBuffer');

const log =     Log.logger ('lib:http-proxy');
const wirelog = Log.logger ('wire:upstream');
...
class HttpProxy {
  ...
  _on_proxyReq (proxyReq, req, res, options) {
    ...
    if (options._is_introspect_activated) {
      ...
      _.each (proxyReq.getHeaders(), (v, k) => req._introspect.req.headers[k] = v);
    }
    ...
    // set a connection timeout if socket not reused/connected
    if (proxyReq.socket.pending) {
      var t = setTimeout(() => {
    ...
    proxyReq.on ('error', err => {
      ...
      if (!(res.headersSent)) {
        if (proxyReq.writableFinished) {
          log.error('proxy upstream error: [%s] %j (res headers NOT sent, proxyReq flushed): respond 502', req.id, err);
          res.writeHead (502, {'Content-Type': 'text/plain'});
          res.end ('request proxied to upstream but that did not respond:\n' + util.inspect(err));
        }
        else {
          log.error('proxy upstream error: [%s] %j (res headers NOT sent, proxyReq NOT flushed): try next upstream', req.id, err);
        ...
        log.error ('proxy upstream error: [%s] %j (res headers sent): do nothing', req.id, err, res.headersSent);
      }
    });
  ...
  _try_web (req, res) {
    for (;;) {
      if (req._upstream_targets.length == 0) {
        // no more upsteams, relay last error
        log.error('[%s] No more upstreams. Sending 502 response', req.id);

        // TODO add info depending on proxyReq.writableFinished
        return res.status (502).send ('no upstreams available');
      }

      // call this._proxy.web with first target
      const target = req._upstream_targets.shift ();
      const upstream = req._upstream.shift ();

      if ((upstream.check) && (!upstream.check.ok)) {
        log.verbose ('[%s] skip target %s (upstream %s, DOWN)', req.id, target, upstream.url);
      }
      else {
        req._upstream_opts.target = target;
        req._upstream_opts.upstream = upstream;
        log.verbose ('[%s] try target %s (upstream %s)', req.id, target, upstream.url);

        return this._proxy.web (req, res, req._upstream_opts);
      }
    }
  }
