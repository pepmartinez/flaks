    ...
    */
    ...
    _.each (_.get (this._config, 'http.routes', {}), (v, k) => {
      ...
      this._paths[k].target.forEach (t => {
        ...
        const check_port = _.get (t, 'check.port');
        if (check_path) {
      ...
      // set lb method
      switch (this._paths[k].lb) {
        case 'seq':
          this._paths[k].lb_method = VHost.s_lb_method_seq;
          break;

        case 'rand':
          this._paths[k].lb_method = VHost.s_lb_method_rand;
          break;

        default:
          this._log.warn ('unknown lb method [%s] on route %s. Using default "seq"', this._paths[k].lb, k);
          this._paths[k].lb_method = VHost.s_lb_method_seq;
          break;
      }
    });
