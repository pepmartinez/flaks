...
class VHost {
  ...
  _check_target (target) {
    ...
    get.concat (opts, (err, res, data) => {
      if (err) return this._check_target_ko (target, err);
      if (+(res.statusCode) != 200) return this._check_target_ko (target, {statusCode: res.statusCode, body: data});
      this._check_target_ok (target);
    });
  }
}
