  ...
  describe('introspection', () => {
    it('records introspection correctly with no agent', done => {
      flaks (config, (err, context) => {
        ...
        async.series ([
          cb => request(context.app).get('/not/defined').end (cb),
          cb => request(context.app).post('/c/nowhere/to/go').send('ddfgdgdgdgdf').type('text').end (cb),
          cb => request(context.app).post('/d/noname/to/go').send('ddfgdgdgdgdf').type('text').end (cb),
          cb => request(context.app).post('/b/h').query ({a:1, b:'666'}).send('ddfgdgdgdgdf').set ({'x-request-id': 'qwertyuiop'}).type('text').end (cb),
          cb => request(context.app).post('/b/h').query ({a:1, b:'666'}).send('ddfgdgdgdgdf').set ({'x-request-id': 'asdfghjkl'}).type('text').end (cb),
        ], err => {
          if (err) return done (err);

//          console.log (require('util').inspect (context.proxy.status (), {depth: null, colors: true}));
          context.proxy.status ().should.match (
    ...
    it('records introspection correctly with agent', done => {
      flaks (config_with_agents, (err, context) => {
        ...
        async.series ([
          ...
          cb => request(context.app).post('/c/nowhere/to/go').send('ddfgdgdgdgdf').type('text').end (cb),
          cb => request(context.app).post('/d/noname/to/go').send('ddfgdgdgdgdf').type('text').end (cb),
          cb => request(context.app).post('/b/h').query ({a:1, b:'666'}).send('ddfgdgdgdgdf').set ({'x-request-id': 'qwertyuiop', Connection: 'keep-alive'}).type('text').end (cb),
          cb => request(context.app).post('/b/h').query ({a:1, b:'666'}).send('ddfgdgdgdgdf').set ({'x-request-id': 'asdfghjkl', Connection: 'keep-alive'}).type('text').end (cb),
        ], err => {
          if (err) return done (err);
